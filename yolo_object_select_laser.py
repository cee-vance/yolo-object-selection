...
last_sent_position = None  # (servo_x, servo_y)
...
def pixel_to_angle(x, y, frame_width=640, frame_height=480, hfov=60, vfov=45):
    ...
    angle_y = -(dy / frame_height) * vfov  # Y-axis stays inverted
...
arduino = serial.Serial('COM5', 9600)  # Adjust COM port
...
while True:
    ...
    for tag, data in previous_objects.items():
        ...
        if selected_tag in previous_objects:
            x1, y1, x2, y2 = previous_objects[selected_tag]['bbox']
            x_center = (x1 + x2) // 2
            y_center = (y1 + y2) // 2

            # Convert to angles and servo positions
